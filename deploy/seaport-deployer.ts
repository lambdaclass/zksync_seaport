...
const PRIVATE_KEY = `0x${process.env.WALLET_PRIVATE_KEY}` || "";
...
export default async function () {
  ...
  const salt =
    "0x00000000000000000000000000000000000000000000fcc21169b803f25d2210";
  ...
  const immutableCreate2 = await deployContract(
    immutableCreate2FactoryInterfaceArtifact
  );
  ...
  const seaport = await deployContract(seaportArtifact, [
    coduitController.address,
  ]);

  // Deploy Helpers contract
  const transferHelperArtifact = "TransferHelper";
  const transferHelper = await deployContract(transferHelperArtifact, [
    coduitController.address,
  ]);
  ...
  const transaction = await provider.getTransaction(
    transferHelper.deployTransaction.hash
  );
  const creationCode = transaction.data;

  const transferHelper_address = await immutableCreate2.safeCreate2(
    bytesFormat,
    creationCode,
    { gasLimit: 1000000 }
  );
